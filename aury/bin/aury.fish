...
aury Aury, quero que você atualize, otimize o sistema e baixe o firefox.
aury remover vlc
aury procurar steam
...
function __aury_normalize_token --argument-names tok
    ...
    switch $t
        ...
        case o a os as um uma uns umas de dos das com sobre ao aos à às por pro porgentileza pfvr porfa gentileza favor porfavor porfavorzinho me mim comigo ai aí aury que você voce vc cê ce te contigo pra-mim pramim
            echo __IGNORE__
        ...
        case ajuda ajudar help socorro manual comandos comando
            echo ajuda

        case reload recarregar recarrega recarregue reiniciar
            echo reload

        case dev developer desenvolvedor diagnostico diagnóstico validar valida verificar verifica conferir confere debug codigo código
            echo dev
        ...
        case remover remove remov remova deletar deleta delete excluir apagar apague desinstalar desinstala desinstale uninstall removr remvoe tirar retira
            echo remover
        ...
        case ver veja mostra mostrar mostre exibir listar checar checa consulte consultar visualizar olhar olhe monitorar diga dizer fale informar informe informa
            echo ver
...
function __aury_is_aux_token --argument-names tok
    if contains -- $tok quero queria preciso gostaria pode poderia poderiame poderia-me conseguimos consegue conseguiria tenta tentar tente favor gentileza porfavor pfvr porfa porfavorzinho ajuda ajude ajudar
        return 0
    end
...
function __aury_should_treat_help_as_aux
    set -l words $argv

    if not contains -- ajuda $words
        return 1
    end
...
function __aury_trim_conversational_tail
    set -l words $argv

    while test (count $words) -gt 0
        set -l last_norm (__aury_normalize_token $words[-1])

        if test "$last_norm" = "__IGNORE__"
            set words $words[1..-2]
            continue
        end

        if __aury_is_aux_token $last_norm
            set words $words[1..-2]
            continue
        end

        if __aury_is_connector_token $last_norm
            set words $words[1..-2]
            continue
        end
...
function __aury_collect_intent_indexes
    ...
    while test $i -le (count $words)
        ...
        if contains -- $tok (__aury_intents)
            if test "$tok" = "status"
                if test $i -gt 1
                    set -l prev_index (math $i - 1)
                    set -l prev_tok $words[$prev_index]

                    if contains -- $prev_tok ver
                        set i (math $i + 1)
                        continue
                    end
                end
...
function __aury_interpret_action
    ...
    while test $j -le (count $filtered_norm)
        ...
        if test "$tok" = "ajuda"; and __aury_should_treat_help_as_aux $filtered_norm
            set j (math $j + 1)
            continue
        end
    ...
    set interpreted_norm (__aury_clean_segment_edges $interpreted_norm)
    if test (count $interpreted_orig) -gt 0
        set interpreted_orig (__aury_trim_conversational_tail $interpreted_orig)
    end
...
function __aury_split_actions
    ...
    while test $i -lt $total
        set -l current_token $argv_tokens_global[$i]
        set -l current_norm (__aury_normalize_token $current_token)
        ...
        else if contains -- $current_norm (__aury_intents)
...
function __aury_after_normalized_keyword --argument-names keyword
    ...
    set -l words $orig_words_global[(math $idx + 1)..-1]
    set -l trimmed (__aury_trim_conversational_tail $words)
...
function __aury_exec_packages
    ...
    switch $intent
        ...
        case remover
            ...
            if pacman -Qi -- $pkg >/dev/null 2>/dev/null
                sudo pacman -Rns -- $pkg
                return 0
            end

            if type -q paru
                if paru -Qi -- $pkg >/dev/null 2>/dev/null
                    paru -Rns -- $pkg
                    return 0
                end
            end
...
function aury
    ...
    set -l raw_text_direct (string join " " -- $argv)
    set -l lowered (string lower -- $raw_text_direct)

    # Bypass mínimo e isolado para:
    #   Aury, me ajuda a copiar X para Y
    #   Aury, me ajuda a mover X para Y
    set -l help_copy_groups (string match -r --groups-only '^[[:space:]]*(?:aury[[:punct:]]*[[:space:]]*)?me ajuda a copiar[[:space:]]+(.+?)[[:space:]]+para[[:space:]]+(.+?)[[:space:][:punct:]]*$' -- $lowered)
    if test (count $help_copy_groups) -eq 2
        set -l orig_groups (string match -r --groups-only '^[[:space:]]*(?:Aury[[:punct:]]*[[:space:]]*)?me ajuda a copiar[[:space:]]+(.+?)[[:space:]]+para[[:space:]]+(.+?)[[:space:][:punct:]]*$' -- $raw_text_direct)
        if test (count $orig_groups) -eq 2
            set -l src (string trim -- $orig_groups[1])
            set -l dst (string trim -- $orig_groups[2])

            if not test -e "$src"
                __aury_msg_error "origem não encontrada: $src"
                return 0
            end

            set -l dest_dir (dirname -- $dst)
            if test "$dest_dir" != "."
                mkdir -p -- $dest_dir
            end
            ...
            if test -d "$src"
                cp -r -- "$src" "$dst"
                set -l item_type pasta
            else
                cp -- "$src" "$dst"
                set -l item_type arquivo
            end

            if test $status -eq 0
                if test "$item_type" = "pasta"
                    echo "📁 pasta copiada: $src → $dst"
                else
                    echo "📄 arquivo copiado: $src → $dst"
                end
            else
                __aury_msg_error "erro ao copiar"
            end
    ...
    set -l help_move_groups (string match -r --groups-only '^[[:space:]]*(?:aury[[:punct:]]*[[:space:]]*)?me ajuda a mover[[:space:]]+(.+?)[[:space:]]+para[[:space:]]+(.+?)[[:space:][:punct:]]*$' -- $lowered)
    if test (count $help_move_groups) -eq 2
        set -l orig_groups (string match -r --groups-only '^[[:space:]]*(?:Aury[[:punct:]]*[[:space:]]*)?me ajuda a mover[[:space:]]+(.+?)[[:space:]]+para[[:space:]]+(.+?)[[:space:][:punct:]]*$' -- $raw_text_direct)
        if test (count $orig_groups) -eq 2
            set -l src (string trim -- $orig_groups[1])
            set -l dst (string trim -- $orig_groups[2])
            ...
            if test $status -eq 0
                if test "$item_type" = "pasta"
                    echo "📁 pasta movida: $src → $dst"
                else
                    echo "📄 arquivo movido: $src → $dst"
                end
            else
                __aury_msg_error "erro ao mover"
            end
