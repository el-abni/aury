...
Este documento descreve o estado público real sustentado pela **💜 Aury v1.6.1** no repositório canônico local.

A proposta do projeto não mudou: Aury continua recebendo frases humanas, fechando uma leitura segura e escolhendo entre executar, bloquear ou cair em fallback honesto. O que mudou na v1.6.1 é a distribuição de responsabilidade entre o adaptador Fish e o núcleo Python.

## Entrada pública e base instalada

Na v1.6.1, a entrada pública continua sendo Fish:
...
No recorte atual da v1.6.1, o núcleo Python já cobre `help`, `version`, `dev`, preparação/análise interna, múltiplas ações no diagnóstico e um conjunto inicial de execuções normais já migradas.
Entre elas estão, por exemplo, busca de pacote, IP, ping, velocidade da internet e algumas leituras simples de sistema.
...
Neste ponto da v1.6.1, continuam no adaptador Fish:
...
Isso é deliberado. A v1.6.1 não promete rewrite total; ela promete coerência entre a superfície pública e a base já migrada.

## Limites honestos

- a linha pública 1.6.1 já é híbrida; documentá-la como “só Fish” ficou incorreto
...
A **💜 Aury v1.6.1** continua tendo entrada pública em Fish, mas já não pode ser descrita como uma base exclusivamente Fish. O estado real agora é:
