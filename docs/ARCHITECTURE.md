...
Este documento descreve o estado público real sustentado pela **💜 Aury v1.9.3** no repositório canônico local.

A proposta do projeto não mudou: Aury continua recebendo frases humanas, fechando uma leitura segura e escolhendo entre executar, bloquear ou cair em fallback honesto. O que a linha 1.6.x consolidou foi a distribuição de responsabilidade entre o adaptador Fish e o núcleo Python. A v1.7.0 abriu e estabilizou a superfície pública híbrida atual; a v1.8.0 endureceu o contrato observável dessa mesma base; a v1.9.0 fechou a base híbrida contida; a v1.9.1 abriu a frente inicial de compatibilidade Linux com perfil mínimo de host e política de pacote por família; a v1.9.2 fechou o hardening representacional curto de `aury dev`; e a v1.9.3 retoma a saga de compatibilidade Linux com OpenSUSE mutável em pacote do host, sem reabrir parser nem abrir nova frente geral de produto.

## Entrada pública e base instalada

Na v1.9.3, a entrada pública continua sendo Fish:
...
No recorte atual da v1.9.3, o núcleo Python já cobre `help`, `version`, `dev`, preparação/análise interna, múltiplas ações no diagnóstico e um conjunto explícito de execuções normais já sustentadas diretamente.
...
Nesse domínio, o runtime Python já concentra a decisão de backend, o bloqueio honesto por perfil de host e o endurecimento operacional incremental desta linha: ausência de resultado em busca, no-op honesto e confirmação de estado quando o backend suportado permite isso. Em OpenSUSE mutável, `zypper` entra como backend real de busca e `sudo + zypper` entra como backend real de instalação e remoção, com confirmação de estado por `rpm -q` em `instalar` e `remover`.
Na leitura diagnóstica de `aury dev`, a v1.9.3 preserva a linguagem pública auditável, continua expondo o perfil mínimo do host quando a ação entra no domínio de pacote e carrega adiante o fechamento curto local da v1.9.2 sem reabrir uma frente ampla de parser.
...
Na v1.9.3, continuam no adaptador Fish:
...
Isso é deliberado. A v1.9.3 preserva a compatibilidade inicial no domínio de pacote aberta na v1.9.1, carrega o hardening curto de `aury dev` fechado na v1.9.2 e retoma a saga de compatibilidade Linux com OpenSUSE mutável sem prometer migração ampla da execução normal do domínio de arquivos nem suporte cross-distro geral.
...
- o perfil mínimo de host Linux cobre família, mutabilidade e backends centrais de pacote; OpenSUSE mutável entra com recorte útil contido em `procurar`, `instalar` e `remover`, enquanto Atomic permanece bloqueado nesta fase
- a v1.9.3 mantém compatibilidade Linux contida só no domínio de pacote; ela não deve ser descrita como suporte cross-distro amplo nem como paridade total entre famílias
- a v1.9.3 não trata hosts Atomic como equivalentes a Fedora mutável
- a v1.9.3 não fecha por inferência casos de `mover arquivo ... para destino nu` quando a leitura correta depende do estado do filesystem
...
A **💜 Aury v1.9.3** continua a linha 1.x com entrada pública em Fish, mas já não pode ser descrita como uma base exclusivamente Fish. O estado real atual é:
...
- política atual de pacote por família Linux: Arch, Debian/Ubuntu e Fedora mutável como Tier 1; OpenSUSE mutável como recorte útil contido; Atomic bloqueado com honestidade
- pacote endurecido no recorte atual: busca honesta, no-op explícito, confirmação de estado e ausência de fallback localista de política no Fish
...
- inferência de destino nu dependente do filesystem conscientemente fora da v1.9.3
