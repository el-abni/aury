...
Este documento descreve o estado público real sustentado pela **💜 Aury v1.9.2** no repositório canônico local.

A proposta do projeto não mudou: Aury continua recebendo frases humanas, fechando uma leitura segura e escolhendo entre executar, bloquear ou cair em fallback honesto. O que a linha 1.6.x consolidou foi a distribuição de responsabilidade entre o adaptador Fish e o núcleo Python. A v1.7.0 abriu e estabilizou a superfície pública híbrida atual; a v1.8.0 endureceu o contrato observável dessa mesma base; a v1.9.0 fechou a base híbrida contida; a v1.9.1 abriu a frente inicial de compatibilidade Linux com perfil mínimo de host e política de pacote por família; e a v1.9.2 fecha o hardening representacional curto de `aury dev` no domínio local já sustentado pelo legado, sem reabrir parser nem abrir nova frente de produto.

## Entrada pública e base instalada

Na v1.9.2, a entrada pública continua sendo Fish:
...
No recorte atual da v1.9.2, o núcleo Python já cobre `help`, `version`, `dev`, preparação/análise interna, múltiplas ações no diagnóstico e um conjunto explícito de execuções normais já sustentadas diretamente.
...
Nesse domínio, o runtime Python já concentra a decisão de backend, o bloqueio honesto por perfil de host e o endurecimento operacional mínimo herdado da v1.9.1: ausência de resultado em busca, no-op honesto e confirmação de estado quando o backend Tier 1 permite isso.
Na leitura diagnóstica de `aury dev`, a v1.9.2 preserva a linguagem pública auditável, continua expondo o perfil mínimo do host quando a ação entra no domínio de pacote e fecha a frente curta local com renomeação localizada, `copiar -> mover`, `copiar -> renomear` e `mover -> renomear` nos recortes explícitos seguros já auditados.
...
Na v1.9.2, continuam no adaptador Fish:
...
Isso é deliberado. A v1.9.2 preserva a compatibilidade inicial no domínio de pacote aberta na v1.9.1 e endurece a leitura representacional curta de `aury dev` no domínio de arquivos sem prometer migração ampla da execução normal desse domínio e sem empurrar o runtime além do necessário.
...
- a v1.9.2 mantém compatibilidade Linux inicial só no domínio de pacote; ela não deve ser descrita como suporte cross-distro amplo nem como paridade total entre famílias
- a v1.9.2 não fecha por inferência casos de `mover arquivo ... para destino nu` quando a leitura correta depende do estado do filesystem
...
A **💜 Aury v1.9.2** continua a linha 1.x com entrada pública em Fish, mas já não pode ser descrita como uma base exclusivamente Fish. O estado real atual é:
...
- inferência de destino nu dependente do filesystem conscientemente fora da v1.9.2
