...
No recorte atual da v1.9.4, o núcleo Python já cobre `help`, `version`, `dev`, preparação/análise interna, múltiplas ações no diagnóstico e um conjunto explícito de execuções normais já sustentadas diretamente.
...
Na v1.9.4, continuam no adaptador Fish:
...
Isso é deliberado. A v1.9.4 preserva a compatibilidade inicial no domínio de pacote aberta na v1.9.1, carrega o hardening curto de `aury dev` fechado na v1.9.2 e consolida o ganho útil de OpenSUSE mutável da v1.9.3 sem prometer migração ampla da execução normal do domínio de arquivos nem suporte cross-distro geral.
...
- o perfil mínimo de host Linux cobre família, mutabilidade e backends centrais de pacote; OpenSUSE mutável entra como Tier 2 útil contido em `procurar`, `instalar` e `remover`, enquanto Atomic permanece bloqueado nesta fase
- a v1.9.4 mantém compatibilidade Linux contida só no domínio de pacote; ela não deve ser descrita como suporte cross-distro amplo nem como paridade total entre famílias
- a v1.9.4 não trata hosts Atomic como equivalentes a Fedora mutável
- a v1.9.4 não fecha por inferência casos de `mover arquivo ... para destino nu` quando a leitura correta depende do estado do filesystem
...
A **💜 Aury v1.9.4** continua a linha 1.x com entrada pública em Fish, mas já não pode ser descrita como uma base exclusivamente Fish. O estado real atual é:
...
- política atual de pacote por família Linux: Arch, Debian/Ubuntu e Fedora mutável como Tier 1; OpenSUSE mutável como Tier 2 útil contido; Atomic bloqueado com honestidade
- pacote endurecido no recorte atual: busca honesta, no-op explícito, distinção entre backend ausente e sonda auxiliar ausente, confirmação de estado e ausência de fallback localista de política no Fish
...
- inferência de destino nu dependente do filesystem conscientemente fora da v1.9.4
