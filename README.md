...
![version](https://img.shields.io/badge/version-v1.9.4-purple)
...
> Estado público real da **v1.9.4**: a linha **1.x** continua incremental como base híbrida deliberadamente contida. Esta release consolida o domínio de pacote por família/host depois da abertura contida de **OpenSUSE mutável** na **v1.9.3**: detecção, política, plano de execução, confirmação de estado, runtime e leitura auditável de `aury dev` passam a contar a mesma história com mais firmeza nas famílias mutáveis já abertas. Arch, Debian/Ubuntu e Fedora mutável permanecem como Tier 1 inicial; OpenSUSE mutável permanece como Tier 2 útil contido em `procurar`, `instalar` e `remover`; Atomic e Universal Blue continuam em suporte limitado com bloqueio honesto. `update` / `optimize` multi-distro, `rpm-ostree` e a inferência representacional de `mover arquivo ... para destino nu` dependente de filesystem continuam conscientemente fora. O gate final mínimo canônico da linha 1.x continua sendo `bash tests/release_gate_minimo.sh`.
...
A instalação pública atual da v1.9.4 usa o script do próprio repositório:
...
## Contrato público mínimo da v1.9.4
...
- na v1.9.4, `aury dev <frase>` continua sendo o relatório canônico da linha 1.x sem prometer paridade total com toda formulação histórica do legado.
- na v1.9.4, `criar arquivo` e `criar pasta` seguem com rota Python explícita no modo normal, inclusive quando a leitura `dev` fecha esse mesmo micro-recorte como suportado.
- na v1.9.4, a frente curta local de `aury dev` fechada na v1.9.2 permanece incorporada, sem abrir nova rodada ampla de hardening de arquivos nesta release.
- na v1.9.4, `procurar`, `instalar` e `remover` no domínio de pacote continuam dependendo do perfil mínimo do host Linux, de um backend explícito por família e de confirmação honesta de estado quando a ação muta o host.
- na v1.9.4, o recorte útil de pacote cobre Arch e derivadas, Debian/Ubuntu e derivadas, Fedora mutável e OpenSUSE mutável no host; Atomic permanece bloqueado com honestidade.
- na v1.9.4, OpenSUSE mutável permanece como Tier 2 útil contido: usa `zypper` para busca, `sudo + zypper` para instalação/remoção e `rpm -q` como sonda de confirmação em `instalar` e `remover`.
- na v1.9.4, backend ausente e ferramenta auxiliar de confirmação ausente ficam distintos na superfície pública; pacote não volta a improvisar fallback localista fora desse contrato.
...
## Compatibilidade Linux atual na v1.9.4
...
- A v1.9.4 não promete tradução de nomes de pacote, paridade total entre famílias nem suporte cross-distro amplo.
...
- a v1.9.4 não promete compatibilidade simétrica entre famílias Linux nem update/optimize multi-distro
- a v1.9.4 não trata Atomic como host mutável normal de pacote
- a v1.9.4 não infere representacionalmente `mover arquivo ... para destino nu` quando a leitura correta depende do estado do filesystem
...
- a v1.9.0 fechou a base híbrida pública contida; a v1.9.1 abriu a primeira release pública de compatibilidade Linux; a v1.9.2 fechou o hardening representacional curto de `aury dev`; a v1.9.3 abriu OpenSUSE mutável no pacote do host; a v1.9.4 consolida esse mesmo domínio por família/host
...
- o domínio de pacote permanece endurecido: busca sem resultado, backend ausente, ferramenta auxiliar de confirmação ausente, no-op e confirmação de estado saem com superfície pública honesta; o Fish não volta a concentrar política de pacote
- Arch, Debian/Ubuntu e Fedora mutável entram como Tier 1 inicial de pacote; OpenSUSE mutável entra como Tier 2 útil contido com backend real; Atomic continua em suporte limitado com bloqueio honesto
- a inferência de `mover arquivo ... para destino nu`, quando dependente do filesystem, permanece conscientemente fora da v1.9.4 por honestidade representacional
...
- a v1.9.4 consolida esse domínio por família/host, sem abrir compatibilidade ampla nem nova frente geral de produto
